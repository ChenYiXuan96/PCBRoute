...
class RolloutBaseline(Baseline):
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        if dataset is not None:
            if len(dataset) != self.opts.val_size:
                ...
                dataset = None
            elif (dataset[0] if self.problem.NAME in ['tsp', 'PcbRoute'] else dataset[0]['loc']).size(0) != self.opts.graph_size:
        ...
        if dataset is None:
            ...
            self.dataset = self.problem.make_dataset(
                ...
                normalize=self.opts.normalize_input_reward)
    ...
    def wrap_dataset_BC(self, dataset, demos, rewards):
        print("Wrapping dataset...")
        # Need to convert baseline to 2D to prevent converting to double, see
        # https://discuss.pytorch.org/t/dataloader-gives-double-instead-of-float/717/3
        return BaselineDatasetBC(dataset,
                                 # rollout(self.model, dataset, self.opts).view(-1, 1),
                                 torch.tensor(demos, dtype=torch.long),
                                 torch.tensor(rewards, dtype=torch.float).view(-1, 1))

    def unwrap_batch(self, batch):
        return batch['data'], batch['baseline'].view(-1)  # Flatten result to undo wrapping as 2D

    def unwrap_batch_BC(self, batch):
        return batch['data'], batch['demo'], batch['demo_reward'].view(-1)  # Flatten result to undo wrapping as 2D

    def eval(self, x, c):
        # Use volatile mode for efficient inference (single batch so we do not use rollout function)
        with torch.no_grad():
            v, _ = self.model(x, normalize=self.opts.normalize_input_reward)
...
class BaselineDatasetBC(Dataset):

    def __init__(self, dataset=None, demos=None, demos_reward=None):
        super(BaselineDatasetBC, self).__init__()

        self.dataset = dataset
        self.demos = demos
        self.demos_reward = demos_reward
        # assert (len(self.dataset) == len(self.baseline))
        assert (len(self.dataset) == len(self.demos))
        assert (len(self.dataset) == len(self.demos_reward))

    def __getitem__(self, item):
        return {
            'data': self.dataset[item],
            # 'baseline': self.baseline[item],
            'demo': self.demos[item],
            'demo_reward': self.demos_reward[item]
        }
